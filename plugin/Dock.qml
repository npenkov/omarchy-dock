          ...
          Row {
            ...
            Repeater {
              ...
              delegate: Item {
                ...
                readonly property string label: root.itemLabel(modelData, entry)
                // A Nerd Font glyph standing in for an icon file. Plenty of
                // worthwhile dock entries — a script, an RDP session, a kill
                // switch — have no icon on disk to point at.
                readonly property string glyph: Util.isPlainObject(modelData) && modelData.glyph
                  ? String(modelData.glyph) : ""

                // Optical size correction for this item, clamped so a bad
                // value can't blow an icon out of the dock.
                ...
                // Glyph ink is measurable, unlike an icon's alpha, so glyphs
                // normalise themselves: measure the tight bounding box at a
                // reference size, then pick the size that lands the ink at
                // the target fraction of the slot.
                TextMetrics {
                  id: glyphMetrics
                  font.family: Style.font.resolvedFamily
                  font.pixelSize: root.slot
                  text: cell.glyph
                }

                readonly property int glyphSize: {
                  var ink = glyphMetrics.tightBoundingRect.height
                  if (!(ink > 0)) return Math.round(root.slot * 0.66)
                  var target = root.slot * root.glyphInkTarget * cell.iconScale
                  return Math.max(1, Math.round(root.slot * target / ink))
                }

                width: root.cellWidth(modelData)
                height: root.slot
                ...
                Rectangle {
                  ...
                  height: Math.round(root.slot * 0.6)
                  color: Util.alpha(Color.popups.text, 0.25)
                }

                // Tile and artwork share one parent so the hover magnify
                // scales them together instead of the icon sliding around
                // inside a stationary tile.
                Item {
                  id: art
                  visible: !cell.isSpacer
                  anchors.centerIn: parent
                  ...
                  // Grows from its base so the icon lifts out of the dock
                  // rather than drifting through it.
                  transformOrigin: Item.Bottom
                  scale: (root.magnify && iconHover.hovered) ? 1.18 : 1.0
                  Behavior on scale {
                    NumberAnimation { duration: 120; easing.type: Easing.OutCubic }
                  }

                  // The uniform container. Icons come in circles, squares and
                  // bare glyphs; a tile behind every one of them is what makes
                  // a row of mismatched artwork read as a single set.
                  //
                  // In "button" style the tile is painted with the same
                  // tokens as qs.Ui.Button — [controls] normal fill/border at
                  // rest, hover-cursor fill/border under the pointer — so
                  // whatever a theme does to its buttons happens here too.
                  BorderSurface {
                    id: tile
                    visible: root.tiles
                    anchors.fill: parent
                    radius: root.tileRadiusPx
                    readonly property bool button: root.tileStyle === "button"
                    readonly property bool hot: iconHover.hovered
                    color: !button ? Util.alpha(Color.popups.text, root.tileOpacity)
                  ...
                  // Inside a tile the artwork sits inset; without one it uses
                  // the whole slot as before.
                  readonly property int box: Math.round(
                    root.slot * (root.tiles ? root.tileInset : 1.0) * cell.iconScale)

                  Text {
                    visible: cell.glyph !== ""
                    anchors.centerIn: parent
                    text: cell.glyph
                    color: iconHover.hovered ? Color.accent : root.glyphColor
                    Behavior on color { ColorAnimation { duration: 120 } }
                    opacity: root.iconOpacity
                    font.family: Style.font.resolvedFamily
                    font.pixelSize: root.tiles
                      ? Math.round(cell.glyphSize * root.tileInset)
                      : cell.glyphSize
                  }
                  ...
                  Image {
                    ...
                    height: art.box
                    opacity: root.iconOpacity
                    source: (cell.isSpacer || cell.glyph !== "") ? "" : root.itemIcon(cell.modelData, cell.entry)
                    // Oversampled so the hover scale stays crisp on raster icons.
                    sourceSize.width: root.slot * 2
                    sourceSize.height: root.slot * 2
                ...
                HoverHandler {
                  ...
                  onHoveredChanged: {
                    if (hovered) {
                      root.hoveredLabel = cell.label
                      // Window coordinates: the tooltip is a sibling of the
                      // hit area, not of the row this cell sits in.
                      root.hoveredCenterX = hitArea.x + card.x + row.x + cell.x + cell.width / 2
                    } else if (root.hoveredLabel === cell.label) {
                      root.hoveredLabel = ""
                    }
                  }

                  Component.onDestruction: if (hovered && root.hoveredLabel === cell.label) root.hoveredLabel = ""
                }

                TapHandler {
                  enabled: !cell.isSpacer
                  onTapped: root.activate(cell.modelData, cell.entry)
                }
              }
            }
          }
